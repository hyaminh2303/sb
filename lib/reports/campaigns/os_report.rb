module Reports::Campaigns::OsReport
  ...
  def init_report(options)
    ...
    @options = options
    query TrackingModels::OsTracking, "#{TrackingModels::OsTracking.table_name}.operating_system_id, (SELECT name FROM operating_systems WHERE id = #{TrackingModels::OsTracking.table_name}.operating_system_id) as name, SUM(views) as views, SUM(clicks) as clicks", 'name', nil
  end
