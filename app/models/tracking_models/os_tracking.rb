module TrackingModels
  class OsTracking < ActiveRecord::Base
    ...
    def unique_log
      if self.new_record?
        unless self.class.where(campaign_id: campaign_id, operating_system_id: operating_system_id, date: date, banner_id: banner_id).count == 0
    ...
    def date_between_campaign_start_end
      if date.nil? || date < campaign.start_date || date > campaign.end_date
